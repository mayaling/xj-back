<template>
    <div class="table" v-loading="loading" element-loading-text="加载中">
        <div class="container">
            ...
            <!-- <div class="mgb20">
                <router-link :to="{path:'addcanal'}"><el-button type="primary">+新增回复</el-button></router-link>
            ...
            <el-table :data="tableData" border size="medium" ref="multipleTable"  stripe>
                ...
                <el-table-column prop="article_name" label="所属文章" align="center"></el-table-column>
                ...
                <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                        <el-button type="text" @click="checkdetail(scope,row)">点击查看回复详情</el-button>
                        <el-dialog title="回复详情" :visible.sync="dialogTableVisible">
                            <el-row :gutter="20" style="margin-bottom: 20px;" class="showtable">
                              ...
                              <template v-for="item in commentdetaildata">
                                <el-row :gutter="20" style="margin-bottom: 20px;" class="showtable">
                                    ...
                                    <el-col :span="16">
                                      <el-card shadow="hover">
                                          回复内容: <span>{{item.content}}</span>
                                      </el-card>
                                    </el-col>
                                  </el-row>
                              </template>
                             
                            <!-- <el-table :data="commentdetaildata"  border>
                                <el-table-column property="from_name" label="回复人">
                                </el-table-column>
                                <el-table-column property="to_name" label="被回复人"></el-table-column>
    ...
    export default {
        ...
        methods: {
          checkdetail(scope,row){
                  ...
                  this.$get('article-comments/'+scope.row.pid).then((res) => {
                      console.log(res)
            ...
            getlistdata(){
                this.$get('article-comments?sort=-id&is_child=1').then((res) => {
                    console.log(res)
                    if(res.code===1){
                        this.tableData = res.info.items;
                        this.pages = res.info._meta.pageCount;
            ...
            handleCurrentChange(val) {
                this.loading = true
                this.$get('article-comments?sort=-id&is_child=1', {
                    page: val,
        ...
            },
            ...
            handleStatus(index,row){
                ...
                this.$put('article-comments/'+row.id, {
                    del: row.del,
                }).then((res) => {
                    this.loading = false
                    this.search1()
            ...
            handleEdit(scope,row){
                // console.log(scope)
                this.$del('article-comments?sort=-id&is_child=1/'+scope.id, {
                }).then((res) => {
                    if (res.code === 1) {
                        this.$message({
                            message:"删除成功",
