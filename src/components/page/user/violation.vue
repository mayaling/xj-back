<template>
        <div class="table" v-loading="loading" element-loading-text="加载中">
            <div class="container">
                <div class="container-title">违规用户</div>
                <div class="mgb20">
                ...
                <el-table :data="tableData" border size="medium" ref="multipleTable">
                    ...
                    <el-table-column prop="status" label="状态" align="center">
                        <template slot-scope="scope">
                            <span v-if="showEdit[scope.$index]" @click="handelselect(scope,row)">{{scope.row.status}}</span>
                            <el-select v-model="value" placeholder="请选择"  v-if="!showEdit[scope.$index]"  @change="handelselect(scope,row)">
                                <el-option
      ...
      export default {
        ...
        methods: {
          ...
          //   },
            ...
            handelselect(scope,row){
                console.log(scope.$index)
                // console.log(scope[scope.$index])
                // console.log(scope.$index)
                // console.log(!scope.row.flag)
                // this.flag = !this.flag
                // scope[scope.$index].row.flag = !scope[scope.$index].row.flag
                // 如果表格处理了的话，就是点击单条只是单行改变
                // scope.row.flag = !scope.row.flag
                this.showEdit[scope.$index] = true;
                // scope.row.status = !scope.row.status
                // this.tableData.flag = scope.row.flag
                // this.tableData.flag = !this.tableData.flag
                // this.getlistdata()
