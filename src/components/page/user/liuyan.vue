<template>
    <div class="table" v-loading="loading" element-loading-text="加载中">
        <div class="container">
            <div class="container-title">用户留言</div>
...
export default {
    ...
    methods: {
        getlistdata(){
            axios.get(this.baseurl+"general/get-feedback").then((res)=>{
                if(res.data.code===1){
                    this.tableData = res.data.info.items;
                    this.pages = res.data.info._meta.pageCount;
                    for(var i in this.tableData){
                        if(this.tableData[i].type===1){
                            this.tableData[i].type = "页面展示"
                        }else if(this.tableData[i].type===2){
                            this.tableData[i].type = "产品问题"
                        }else{
                            this.tableData[i].type = "操作问题"
                        }
                    }
            ...
            // this.$get('general/get-feedback').then((res) => {
            //     console.log(res)
            //     if(res.code===1){
        ...
        handleStatus(index,row){
            ...
            this.$put('general/get-feedback/'+row.id, {
                status: row.status,
            }).then((res) => {
                this.loading = false
                this.search1()
        ...
        handleEdit(scope,row){
            // console.log(scope)
            this.$del('general/get-feedback/'+scope.id, {
            }).then((res) => {
                if (res.code === 1) {
                    this.$message({
                        message:"删除成功",
        ...
        handleCurrentChange(val) {
            this.loading = true
            axios.get(this.baseurl+"general/get-feedback",{ page: val,}).then((res)=>{
                if(res.data.code===1){
                    this.pages = res.data.info._meta.pageCount;
