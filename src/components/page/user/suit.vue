<template>
        <div class="table" v-loading="loading" element-loading-text="加载中">
            <div class="container">
                <div class="container-title">马甲用户列表</div>
                <div class="mgb20">
                    ...
                    <el-row style="float: left;margin-bottom:20px;">
                            ...
                            <el-col :span="6" offset="1">
                                    <el-select v-model="p_status" placeholder="用户状态" @change="search" clearable="true">
      ...
      export default {
        ...
        methods: {
            getlistdata(){
                this.$get('user/get-user?type=suit').then((res) => {
                    console.log(res)
          ...
          handleModifyStatus(row, status) {
            ...
            if(row.status==='禁用') {
              ...
              if(this.status == 0){
                  ...
                  this.$put('user/get-user?type=suit/'+row.id, {
                      status: this.status,
                  }).then((res) => {
                      this.loading = false
                      // this.search1()
          ...
          //   },
            ...
            handelchangestatus(scope,row){
                console.log(scope)
                this.$put('user/get-user?type=suit/'+scope.row.id,{
                  status:1
                }).then((res) => {
                    // console.log(res)
                    // if(res.code===1){
            ...
            handleCurrentChange(val) {
                this.loading = true
                this.$get('user/get-user?type=suit', {
                    page: val,
                    // channel_id: this.channel?this.channel:'',
                    active:this.active?this.active:'',
                    status:this.p_status?this.p_status:'',
