                ...
                <el-row type="flex" class="row-bg" justify="space-around">
                    <el-col :span="6">
                            ...
                            <el-form :model="form" ref="form" :rules="rules" class="item-add-list">
                                ...
                                <el-form-item label="创业帮id:" prop="data_id" v-if="showid">
    ...
    <script>
        export default {
            ...
            methods: {
                ...
                // 编辑进来获取表单的内容
                getformdata(){
                    this.$get('business-banner-pcs/'+this.id,).then((res) => {
                        console.log(res)
                        if(res.code===1){
                ...
                onSubmit(formName) {
                    this.$refs[formName].validate((valid) => {
                        if (valid) {
                            this.$post('business-banner-pcs', {
                                type:this.form.type,
                                img_url:this.imageurl1,
                                web_url:this.form.web_url?this.form.web_url:"",
                                data_id:this.form.data_id?this.form.data_id:0
                            }).then((res) => {
                                console.log(res)
                                if(res.code === 1){
                                    this.$router.push("/pcbangbaner")
                                }else{
                                    if(!res.info.name){
                ...
                onEdit(formName) {
                    ...
                    this.$refs[formName].validate((valid) => {
                        if (valid) {
                            this.$put('business-banner-pcs/'+this.id, {
                                type:this.form.type,
