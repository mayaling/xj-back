<template>
        <div class="table" v-loading="loading" element-loading-text="加载中">
            <div class="container">
                ...
                <div class="mgb20">
                    <router-link :to="{path:'editbusinesscard'}"><el-button type="primary">+新增名片</el-button></router-link>
                </div>
                <el-table :data="tableData" border size="medium" ref="multipleTable"  stripe>
                    ...
                    <el-table-column label="操作" align="center">
                        <template slot-scope="scope">
                            <router-link :to="{path:'editbusinesscard',query:{id:scope.row.id}}">
                                <el-button size="mini">
                                   编辑
                                </el-button>
                            </router-link>
    ...
    // import VueClipboard from 'vue-clipboard2'
        ...
        export default {
            ...
            methods: {
                getlistdata(){
                    this.$get('business-card-backgrounds').then((res) => {
                        console.log(res)
                        if(res.code===1){
                ...
                handleCurrentChange(val) {
                    this.loading = true
                    this.$get('business-card-backgrounds', {
                        page: val,
                    }).then((res) => {
                        if (res.code === 1) {
                ...
                handleEdit(scope,row){
                    // console.log(scope)
                    this.$del('business-card-backgrounds/'+scope.id, {
                    }).then((res) => {
                        if (res.code === 1) {
                            this.$message({
                                message:"删除成功",
