<template>
    <div class="table" v-loading="loading" element-loading-text="加载中">
        <div class="container">
            <div class="container-title">权益卡列表</div>
            <div class="mgb20">
                <router-link :to="{path:'editinterestcard'}"><el-button type="primary">+新增权益卡</el-button></router-link>
            </div>
            <el-table :data="tableData" border size="medium" ref="multipleTable"  stripe>
                ...
                <el-table-column prop="con_price" label="连续价格" align="center"></el-table-column>
                <el-table-column prop="pre_price" label="原价" align="center"></el-table-column>
                <el-table-column prop="now_price" label="现价" align="center"></el-table-column>
                <el-table-column prop="y_sku_id" label="sku" align="center"></el-table-column>
                ...
                <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                        <router-link :to="{path:'editinterestcard',query:{id:scope.row.id}}">
                            <el-button size="mini">
                               编辑
                            </el-button>
                        </router-link>
...
// import VueClipboard from 'vue-clipboard2'
    ...
    export default {
        ...
        methods: {
            getlistdata(){
                this.$get('interest-cards').then((res) => {
                    console.log(res)
                    if(res.code===1){
            ...
            handleStatus(index,row){
                ...
                this.$put('interest-cards/'+row.id, {
                    status: row.status,
                }).then((res) => {
                    this.loading = false
                    // this.search1()
            ...
            handleEdit(scope,row){
                // console.log(scope)
                this.$del('interest-cards/'+scope.id, {
                }).then((res) => {
                    if (res.code === 1) {
                        this.$message({
                            message:"删除成功",
