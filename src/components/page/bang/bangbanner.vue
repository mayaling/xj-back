<template>
    <div class="table" v-loading="loading" element-loading-text="加载中">
        <div class="container">
            <div class="container-title">bangbanner</div>
            <div class="mgb20">
                <router-link :to="{path:'editbangbanner'}"><el-button type="primary">+新增bangbanner</el-button></router-link>
            </div>
            <el-table :data="tableData" border size="medium" ref="multipleTable"  stripe>
                ...
                　　</template>
                    ...
                    <el-table-column prop="web_url" label="外部链接" align="center"></el-table-column>
                    <el-table-column prop="share_url" label="分享链接" align="center"></el-table-column>
                    <el-table-column prop="share_title" label="分享标题" align="center"></el-table-column>
                    <el-table-column prop="share_desc" label="分享描述" align="center"></el-table-column>
                <!-- <el-table-column prop="info_logo" label="内页logo" align="center">
                　<template slot-scope="scope">
                　　　　<img :src="scope.row.info_logo" width="40" height="40" class="head_pic"/>
                　　</template>
                </el-table-column> -->
                ...
                <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                        <router-link :to="{path:'editbangbanner',query:{id:scope.row.id}}">
                            <el-button size="mini">
                               编辑
                            </el-button>
                        </router-link>
...
// import VueClipboard from 'vue-clipboard2'
    ...
    export default {
        ...
        methods: {
            getlistdata(){
                this.$get('business-banners').then((res) => {
                    console.log(res)
                    if(res.code===1){
            ...
            handleStatus(index,row){
                ...
                this.$put('business-banners/'+row.id, {
                    status: row.status,
                }).then((res) => {
                    this.loading = false
                    // this.search1()
            ...
            handleEdit(scope,row){
                // console.log(scope)
                this.$del('business-banners/'+scope.id, {
                }).then((res) => {
                    if (res.code === 1) {
                        this.$message({
                            message:"删除成功",
