      ...
      <div class="container-title">新增电台</div>	
        <el-form :model="form" ref="form" enctype="multipart/form-data" :rules="rules" class="item-add-list">
            <el-form-item prop="radio_url" label="上传电台">
                ...
                <!-- <el-upload  :on-preview="handlePreview2" :on-remove="handleRemove2"   :file-list="fileList2" :on-change="handleChange2" :http-request="uploadfile"> 
            ...
            <el-form-item  prop="radio_url" label="电台链接">
                <el-input v-model.trim="form.radio_url" type="text" placeholder="请输入电台链接"></el-input>
            </el-form-item>	 
          <el-form-item  prop="title" label="请输入标题">
            ...
            <!-- <el-form-item  prop="origin" label="请输入来源">
                <el-input v-model.trim="form.origin" type="text" placeholder="请输入来源"></el-input>
            </el-form-item> -->
                  <!-- <el-form-item  prop="participants" label="请输入参与者ID">
                        <el-input v-model.trim="form.participants" type="text" placeholder="请输入参与者ID"></el-input>
                      </el-form-item>	 -->
                      <el-form-item  prop="participants" label="请输入参与者ID">
                        <el-select v-model="form.participants" multiple filterable allow-create default-first-option placeholder="请输入参与者ID" v-loadmore="loadMore" filterable>
                            <el-option v-for="item in userdata" :key="item.id" :label="item.name" :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>	
                    <el-form-item  prop="size" >
                            <el-input v-model.trim="form.size" type="text" v-show="false"></el-input>
                    </el-form-item>	
                    <!-- <el-form-item  prop="type" label="请选择类型">
                        ...
                        <el-form-item  prop="label_id_list" label="请选择标签">
                            <el-select v-model="form.label_id_list" multiple filterable allow-create default-first-option placeholder="请选择标签" @change="addkeywords">
        ...
        <router-link :to="{path:'audiolist'}"><el-button type="primary">取消</el-button></router-link>
        </div>
    </div>
</template>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        ossurl:"http://47.103.61.179:1022/general/get-sts?client_name=backend",
        tempUrl: '', //存上传后的图片url
        dataObj: {}, //存签名信息
        baseAli: '', //后面连接图片url用的，根据自己aili OSS 配制修改
        progress:0,//进度条,
        ossdata:"",
        uploadUrl1:"",
        date:"",
    ...
    methods: {
        ...
        loadMore() {
            ...
            if(_this.page != _this.pageCount){
                _this.page++;
                this.$get('users?sort=-id&page='+_this.page).then((res) => {
                    if (res.code === 1) {
...
　　　　},
        // 编辑进来的内容
      getlistdata(){
        this.$get('radios/'+this.id+'?expand=content,label_id_list').then((res) => {
            console.log(res)
            if(res.code===1){
       ...
       onSubmit(formName) {
        ...
        getossdata(){
            ...
            this.$axios({
                ...
                this.dataObj = res.data.info
                this.baseAli = res.data.info.region+".aliyuncs.com"
            }).catch((err)=>{})
            
            // $axios.get(this.ossurl).then(function (res) {
            //     console.log(res)
            //     console.log(res.data.info)
            //     // this.dataObj = res.data.info
            //     this.ossdata = res.data.info
        ...
        Upload(file) {
            const that = this
            console.log(file)
            this.form.size = (file.file.size)/1024
            ...
            async function multipartUpload () {
                ...
                const fileName = 'radio'+'/' + that.date+'/' +file.file.uid + '.' + exname
                ...
                client(that.dataObj).multipartUpload(fileName, file.file,
                {
                    ...
                    result => {
                        ...
                        that.form.radio_url =  that.tempUrl
